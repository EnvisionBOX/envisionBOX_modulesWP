...
## Continuous trajectory analyes
Note that the above density analysis is not really ideal because the peak is determined within a gesture event we would rather have the nearest peak of the amplitude envelope, rather than the max peak within a gesture event window another cool way is to go for a more continuous approach: what if we look at all the trajectories of the envelope around the moment (-+ 200 milliseconds) that a peak in speed emerged. The dataset we are creating here would be amenable to a GAM analyses for example.

```{r}
mfield <- data.frame() #initialize a data.frame for all the trajectories
for(ID in unique(merged$gesture_ID[!is.na(merged$gesture_ID)])) #loop through all gesture ID's
{
  temp <- subset(merged, gesture_ID==ID)                                      #select a time series chunk having that gesture ID
  merged$time_temp <- merged$time-temp$time[which.max(temp$speed)] #subtract from a temporary time vector in the original TS the timing of the peak speed of that gesture 
  tempTS <- merged[abs(merged$time_temp) < 200 & merged$gesture_ID==ID,]              #now keep only the original TS where 250 millisecond after and before the peak speed
...
p1 <- ggplot(mfield, aes(x=time, y = ENV, color = g_type, group = as.factor(g_ID))) + geom_line(alpha=0.7) + theme_bw() +  ylab('amplitude envelope trajectories')
p2 <- ggplot(mfield, aes(x=time, y = ENV, color = g_type)) + geom_smooth(method = 'gam', size = 2) + theme_bw() + ylab('gam trajectory amplitude envelope')
